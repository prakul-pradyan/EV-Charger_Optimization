...
- **Machine Learning Pipeline**: Natively integrates `scikit-learn` to classify sites dynamically and identify anomalies:
  - **K-Means Clustering (k=4)**: Groups cities into specialized market tiers (*Priority Expansion*, *Emerging Market*, *Saturated Zone*, *Low Viability*) based on multidimensional feature tracking.
  - **DBSCAN Anomaly Detection**: Filters demographic sets to tag localized outliers preventing skewed investment forecasts.
  - Generates real-time Silhouette Scores and Elbow Curve data.
...
While the primary interface is the frontend UI dashboard, the underlying computational engine is completely modular and exposed for external utilization:

- **Run Main Analysis**: `POST /api/analysis/run` (Accepts optional `weight_overrides` payload).
- **Machine Learning Insights**: 
  - `GET /api/ml-insights`: Instantly streams the cached JSON array of ML clustering classifications generated by the most recent dashboard run straight to your browser.
  - `POST /api/ml-insights`: Accepts an array of dictionaries representing EV sites/cities and asynchronously runs `scikit-learn` algorithms against them to output classifications and anomalies.
